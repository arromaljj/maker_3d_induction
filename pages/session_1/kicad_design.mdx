...
2. In the "Board Editor Options" tab, set the board size to 60mm x 40mm (or your preferred size)

### Set Design Rules
1. In the "Board Setup" window, go to the "Design Rules" tab
2. Set the following:
   - Clearance: 0.2mm
   - Track Width: 0.25mm (min), 0.5mm (power)
   - Via Size: 0.8mm
   - Via Drill: 0.4mm
...
1. Position the USB connector at the edge of the board
2. Place the TP4056 charging IC near the USB connector
3. Position the battery connector on the opposite side of the board from the USB connector

### Arrange Major Components
1. Place the ATtiny85 near the center of the board
2. Position the OLED display at the top of the board
3. Arrange the three push buttons in an ergonomic layout, perhaps in a triangle or line

### Place Remaining Components
1. Position the piezo buzzer where it won't interfere with other components
2. Place the power switch in an accessible location
3. Arrange the LEDs so they're visible when the device is assembled
4. Place resistors and capacitors close to their associated components
...
2. Use the back layer (B.Cu) as a ground plane

### Start Routing
1. Begin with power traces from the USB connector to the TP4056
2. Route the battery connections, ensuring they can handle the necessary current
3. Connect the ATtiny85 power and ground pins
4. Route the OLED display connections, keeping the traces short and direct
5. Connect the push buttons and piezo buzzer
6. Route any remaining signal traces
...
2. Ensure all power traces have appropriate width for their current capacity
</Steps>

<Callout type="warning">
  Pay special attention to the power traces for the charging circuit. They need to be wide enough to handle the charging current safely.
</Callout>

## Adding Copper Pours

Adding copper pours can improve power distribution and provide shielding:

<Steps>
### Create Ground Plane
1. Select the B.Cu layer
2. Use the "Add Filled Zones" tool to create a ground pour covering most of the board

### Add Power Pours
1. On the F.Cu layer, create smaller copper pours for VCC and the battery positive connection
2. Ensure these pours are connected to the appropriate nets
</Steps>
...
<Steps>
### Add Text
1. Use the "Add Text" tool to label key components (e.g., buttons, power switch)
